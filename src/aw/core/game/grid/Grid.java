...
public class Grid {
    ...
    /**
     * Ajoute une unit dans la grille
     * @param unit unite a ajouter
     */
    public void addUnit(Unit unit) {
        int location = bijection(unit.getLocation().getX(), unit.getLocation().getY());
        if (this.grid[location].getOptionalUnit().isPresent())
            return;
        this.grid[location].setOptionalUnit(Optional.of(unit));
    }
    ...
    private int bijection(int x, int y) {
        System.out.println(x + " " + y + " " + (x * this.height + y) + " " + this.height);
